    ...
    <div class="w-100 @if (!gs('registration')) form-disabled @endif">
        ...
        <form class="account-form row verify-gcaptcha disableSubmission" action="{{ route('user.register') }}" method="POST">
            ...
            @if (gs('agree'))
                ...
                @endphp
                <div class="form--group custom--checkbox" style="direction: rtl">
            ...
            <div class="form--group" style="direction: rtl">
                <button class="btn btn--base w-100" id="recaptcha" type="submit"> @lang('تسجيل')</button>
            </div>

            <p class="mb-0" style="direction: rtl"> @lang('هل لديك حساب بالفعل؟') <a href="{{ route('user.login') }}">@lang('تسجيل دخول')</a></p>
        </form>
    </div>
@endsection
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style="direction: rtl">
                    <h5 class="modal-title" id="existModalLongTitle">@lang('أنت معنا')</h5>
                    <button class="btn--close w-auto" data-bs-dismiss="modal" type="button"><i class="las la-times"></i></button>
                </div>
                <div class="modal-body" style="direction: rtl">
                    <strong class="text-dark">@lang('لديك حساب بالفعل. الرجاء تسجيل الدخول.')</strong>
                </div>
                <div class="modal-footer" style="direction: rtl">
                    <button class="btn btn--danger btn--sm w-auto" data-bs-dismiss="modal" type="button">@lang('اغلاق')</button>
                    <a class="btn btn--base btn--sm w-auto" href="{{ route('user.login') }}">@lang('تسجيل الدخول')</a>
                </div>
            </div>
        </div>
    </div>
