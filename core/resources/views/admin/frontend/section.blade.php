...
{{--                                                     <label>{{ __(keyToTitle(@$imgKey)) }}</label>
 --}}                                                    <x-image-uploader class="w-100" name="image_input[{{ @$imgKey }}]" :imagePath="frontendImage($key,@$content->data_values->$imgKey,@$section->content->images->$imgKey->size)" id="image-upload-input{{ $loop->index }}" :size="$section->content->images->$imgKey->size" :required="false" />
                                        ...
                                        @endforeach
                                        <div class="@if ($imgCount > 1) col-md-12 @else col-md-8 @endif" style="direction: rtl">
                                            @push('divend')
                                            </div>
                                        @endpush
                                    @else
    ...
    @if (@$section->element)
        <div class="d-flex flex-wrap justify-content-end mb-3">
            <div class="d-inline">
                <div class="input-group justify-content-end" style="direction: rtl">
                    <input type="text" name="search_table" class="form-control bg--white" placeholder="'ابحث هنا...">
                                            ...
                                            <td>
                                                <div class="button--group">
                                                    @if(@$section->element->seo)
                                                        <a href="{{ route('admin.frontend.sections.element.seo', [$key,$data->id]) }}" class="btn btn-sm btn-outline--info"><i class="la la-cog"></i> @lang('إعدادات تحسين محركات البحث')</a>
                                                    @endif
                                                    @if ($section->element->modal)
                                                        ...
                                                        <button class="btn btn-sm btn-outline--primary updateBtn" data-id="{{ $data->id }}" data-all="{{ json_encode($data->data_values) }}" @if (@$section->element->images) data-images="{{ json_encode($images) }}" @endif>
                                                            <i class="la la-pencil-alt"></i> @lang('تعديل')
                                                        </button>
                                                    @else
                                                        <a href="{{ route('admin.frontend.sections.element', [$key, $data->id]) }}" class="btn btn-sm btn-outline--primary"><i class="la la-pencil-alt"></i> @lang('تعديل')</a>
                                                    @endif
                                                    <button class="btn btn-sm btn-outline--danger confirmationBtn" data-action="{{ route('admin.frontend.remove', $data->id) }}" data-question="@lang('هل أنت متأكد من إزالة هذا العنصر؟')"><i class="la la-trash"></i> @lang('إزالة')</button>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> إضافة عنصر @lang(' جديد')</h5>
        ...
        <div id="updateBtn" class="modal fade" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> @lang('تحديث') {{ __(keyToTitle($key)) }} @lang('عنصر')</h5>
                        <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                            <i class="las la-times"></i>
                        </button>
                    </div>
...
@push('breadcrumb-plugins')
    <div class="d-flex flex-wrap justify-content-end gap-2 align-items-center">
        @if (@$section->element)
            @if ($section->element->modal)
                <a href="javascript:void(0)" class="btn btn-sm btn-outline--primary addBtn"><i class="las la-plus"></i>@lang('إضافة جديد')</a>
            @else
                <a href="{{ route('admin.frontend.sections.element', $key) }}" class="btn btn-sm btn-outline--primary"><i class="las la-plus"></i>@lang('إضافة جديد')</a>
            @endif
        @endif
        @if (!empty($templates))
                ...
                <form action="{{ route('admin.frontend.import', $key) }}" method="post">
                    <div class="input-group">
                        ...
                        <select name="template_name" class="form-control form-control-sm border--primary h-auto">
                            <option value="">@lang('اختر واحد')</option>
                            @foreach ($templates as $template)
                                <option value="{{ $template['name'] }}">{{ __(keyToTitle($template['name'])) }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="input-group-text btn btn-sm btn--primary">@lang('استيراد')</button>
                    </div>
                </form>
            </div>
        @endif
