                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <a href="javascript:void(0)" class="buildSlug"><i class="las la-link"></i> @lang('أنشئ رابطًا بسيطًا
')</a>
                                        </div>
...
')</label>
    ...
    <div class="row g-1 g-lg-3">
        <div class="col-6 col-lg-6 col-xl-8">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __($pData->name) }} @lang('صفحة')</h3>
                </div>

                <div class="card-body">
                    <div class="submitRequired bg--warning d-none form-change-alert"><i
                            class="fas fa-exclamation-triangle"></i>
                        @lang('يتعين عليك النقر فوق زر التحديث الآن لتطبيق التغييرات')</div>
                        ...
                        <ol id="page_sections" class="simple_with_drop vertical sec-item">
                            ...
                            @else
                                <li class="empty-state">
                                    <span>@lang('اسحب وأفلِت قسمك هنا')</span>
                                </li>
                            @endif
                        </ol>
                        <button class="btn btn--primary w-100 h-45" type="submit">@lang('تحديث الآن')</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-lg-6 col-xl-4">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">@lang('الأقسام')</h3>
                    <small class="text--primary">@lang('اسحب القسم الذي تريد عرضه على الصفحة إلى الجانب الأيسر.')</small>
                </div>
            ...
            $('[name=slug]').on('input',function(){
                ...
                if (slug) {
                    ...
                    $('.slug-verification').html(`
                        <small class="text--info"><i class="las la-spinner la-spin"></i> @lang('التحقق')</small>
                    `);
                    $.get("{{ route('admin.frontend.manage.pages.check.slug',$pData->id) }}", {slug:slug},function(response){
                        ...
                        if (response.exists) {
                            $('.slug-verification').html(`
                                <small class="text--danger"><i class="las la-times"></i> @lang('الرابط النصي موجود مسبقًا')</small>
                            `);
                        }
                    });
                }else{
