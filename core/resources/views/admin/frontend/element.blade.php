...
')</a>
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn--primary @if(@$section->element->slug && !@$data) disabled @endif w-100 h-45">@lang('تأكيد')</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                $('[name=slug]').on('input',function(){
                    ...
                    if (slug) {
                        ...
                        $.get("{{ route('admin.frontend.sections.element.slug.check',[$key,@$data->id]) }}", {slug:slug},function(response){
                            ...
                            if (response.exists) {
                                closestForm.find('.slug-verification').html(`
                                    <small class="text--danger"><i class="las la-times"></i> @lang('الحلزون موجود بالفعل')</small>
                                `);
                            }
                        });
                    }else{
