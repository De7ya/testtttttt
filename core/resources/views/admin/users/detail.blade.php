        ...
        <div class="col-12">
            ...
            <div class="d-flex flex-wrap gap-3 mt-4">
                ...
                <div class="flex-fill">
                    ...
                    <button type="button" class="btn btn--warning btn--shadow w-100 btn-lg userStatus" data-bs-toggle="modal" data-bs-target="#userStatusModal">
                        <i class="las la-ban"></i>@lang('حظر المستخدم')
                    </button>
                    @else
                    <button type="button" class="btn btn--success btn--shadow w-100 btn-lg userStatus" data-bs-toggle="modal" data-bs-target="#userStatusModal">
                        <i class="las la-undo"></i>@lang('إلغاء حظر المستخدم')
                    </button>
                    @endif
                </div>
            </div>
            ...
            <div class="card mt-30">
                <div class="card-header">
                    <h5 class="card-title mb-0">@lang('معلومات عن') {{$user->fullname}}</h5>
                </div>
                <div class="card-body">
                    <form action="{{route('admin.users.update',[$user->id])}}" method="POST"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-control-label">@lang('الاسم الأخير')</label>
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="form-group ">
                                    <label>@lang('الدولة') <span class="text--danger">*</span></label>
...
{{--                             <div class="col-xl-4 col-12">
 ...
 --}}
                            <div class="col-md-12">
                                <button type="submit" class="btn btn--primary w-100 h-45">@lang('تأكيد')
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><span class="type"></span> <span>@lang('الرصيد')</span></h5>
                ...
                <form action="{{route('admin.users.add.sub.balance',$user->id)}}" class="balanceAddSub disableSubmission" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>@lang('المبلغ')</label>
                            <div class="input-group">
                                <input type="number" step="any" name="amount" class="form-control" placeholder="@lang('يرجى إدخال مبلغ')" required>
                                <div class="input-group-text">{{ __(gs('cur_text')) }}</div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>@lang('ملاحظة')</label>
                            <textarea class="form-control" placeholder="@lang('ملاحظة')" name="remark" rows="4" required></textarea>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        @if($user->status == Status::USER_ACTIVE) @lang('حظر المستخدم') @else @lang('الغاء الحظر') @endif
                    </h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                ...
                <form action="{{route('admin.users.status',$user->id)}}" method="POST">
                    ...
                    <div class="modal-body">
                        @if($user->status == Status::USER_ACTIVE)
                        <h6 class="mb-2">@lang('إذا قمت بحظر هذا المستخدم، فلن يتمكن من الوصول إلى لوحة التحكم الخاصة به.')</h6>
                        <div class="form-group">
                            <label>@lang('السبب')</label>
                            <textarea class="form-control" name="reason" rows="4" required></textarea>
                        </div>
                        @else
                        <p><span>@lang('كان سبب الحظر'):</span></p>
                        <p>{{ $user->ban_reason }}</p>
                        <h4 class="text-center mt-3">@lang('هل أنت متأكد من إلغاء حظر هذا المستخدم؟')</h4>
                        @endif
                    </div>
                    <div class="modal-footer">
                        @if($user->status == Status::USER_ACTIVE)
                        <button type="submit" class="btn btn--primary h-45 w-100">@lang('تأكيد')</button>
                        @else
                        <button type="button" class="btn btn--dark" data-bs-dismiss="modal">@lang('لا')</button>
                        <button type="submit" class="btn btn--primary">@lang('نعم')</button>
                        @endif
                    </div>
                </form>
            </div>
...
@push('breadcrumb-plugins')
    <a href="{{route('admin.users.login',$user->id)}}" target="_blank" class="btn btn-sm btn-outline--primary" ><i class="las la-sign-in-alt"></i>@lang('تسجيل الدخول كمستخدم')</a>
@endpush
