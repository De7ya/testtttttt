...
class SiteController extends Controller
{
    ...
    public function contactSubmit(Request $request)
    {
        ...
        if (!verifyCaptcha()) {
            $notify[] = ['Error', 'كود كابتشا غير صالح'];
        ...
        $notify[] = ['Success', 'تم إنشاء التذكرة بنجاح!'];
    ...
    public function bookTicket(Request $request, $id)
    {
        ...
        if (!auth()->user()) {
            $notify[] = ['error', 'لا يمكنك حجز أي تذاكر بدون تسجيل الدخول.'];
        ...
        if ($date_of_journey->format('Y-m-d') < $today) {
            $notify[] = ['error', 'لا يمكن أن يكون تاريخ الرحلة أقل من تاريخ اليوم.'];
        ...
        if (!empty($trip->day_off)) {
            if (in_array($dayOff, $trip->day_off)) {
                $notify[] = ['error', 'الرحلة غير متاحة لـ… ' . $date_of_journey->format('l')];
                return redirect()->back()->withNotify($notify);
            }
        }
        ...
        if ($booked_ticket->count() > 0) {
            $notify[] = ['error', 'تلك المقاعد محجوزة بالفعل.'];
            return redirect()->back()->withNotify($notify);
        }
        ...
        if (!$can_go) {
            $notify[] = ['error', 'يرجى تحديد نقطة الانطلاق ونقطة الوصول بشكل صحيح.'];
            return redirect()->back()->withNotify($notify);
        }
    ...
    public function ticketSearch(Request $request)
    {
        if ($request->pickup && $request->destination && $request->pickup == $request->destination) {
            $notify[] = ['Error', 'يرجى تحديد نقطة الانطلاق ونقطة الوجهة بشكل صحيح.'];
            return redirect()->back()->withNotify($notify);
        }
        if ($request->date_of_journey && Carbon::parse($request->date_of_journey)->format('Y-m-d') < Carbon::now()->format('Y-m-d')) {
            $notify[] = ['Error', 'تاريخ الرحلة لا يمكن أن يكون أقل من تاريخ اليوم.'];
            return redirect()->back()->withNotify($notify);
        }
