...
class UserController extends Controller
{
    ...
    public function disable2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $user->save();
            $notify[] = ['Success', 'تم إيقاف تفعيل المصادقة الثنائية بنجاح'];
        } else {
            $notify[] = ['Error', 'رمز التحقق خاطئ '];
        }
        return back()->withNotify($notify);
    }
    ...
    public function userDataSubmit(Request $request)
    {
        ...
        if (preg_match("/[^a-z0-9_]/", trim($request->username))) {
            $notify[] = ['info', 'يمكن أن يحتوي اسم المستخدم على أحرف صغيرة وأرقام وعلامة الشرطة السفلية (_) فقط.'];
            $notify[] = ['error', 'لا يُسمح باستخدام أي رموز خاصة أو مسافات أو أحرف كبيرة في اسم المستخدم.'];
            return back()->withNotify($notify)->withInput($request->all());
        }

        $user->country_code = $request->country_code;
        $user->mobile       = $request->mobile;
        $user->gender    = (int)($data['gender'] ?? 1);   // <-- save gender
