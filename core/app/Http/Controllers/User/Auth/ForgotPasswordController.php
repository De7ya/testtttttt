...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetCodeEmail(Request $request)
    {
        ...
        if(!verifyCaptcha()){
            $notify[] = ['error','تم إدخال كود الكابتشا غير صالح.'];
        ...
        if (!$user) {
            $notify[] = ['error', 'تعذر العثور على الحساب'];
            return back()->withNotify($notify);
        }
        ...
        $notify[] = ['Success', 'تم إرسال بريد إعادة تعيين كلمة المرور بنجاح'];
    ...
    public function codeVerify(Request $request){
        ...
        if (!$email) {
            $notify[] = ['error','عذرًا! انتهت صلاحية الجلسة.'];
    ...
    public function verifyCode(Request $request)
    {
        ...
        if (PasswordReset::where('token', $code)->where('email', $request->email)->count() != 1) {
            $notify[] = ['error', 'رمز التحقق غير مطابق'];
            return to_route('user.password.request')->withNotify($notify);
        }
