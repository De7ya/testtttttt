...
class ManageUsersController extends Controller
{
    ...
    {
        $pageTitle = 'driver';
        $emptyMessage = 'لا يوجد سائقون حتى الآن.';
        $users = Driver::paginate(10); // or Driver::all() if you don’t want pagination

        return view('admin.users.driver', compact('pageTitle', 'users', 'emptyMessage'));
    }
    public function store(Request $request)
    {
        ...
        $notify[] = ['success', 'تم حفظ بيانات السائق بنجاح'];
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($exists) {
            $notify[] = ['error', 'رقم الهاتف موجود بالفعل.'];
    ...
    public function addSubBalance(Request $request, $id)
    {
        ...
        if ($request->act == 'add') {
            ...
            $notify[] = ['success', 'تمت إضافة الرصيد بنجاح.'];
        } else {
            if ($amount > $user->balance) {
                $notify[] = ['error', $user->username . ' لا يوجد رصيد كافٍ.'];
                return back()->withNotify($notify);
            }
            ...
            $notify[] = ['success', 'تم خصم الرصيد بنجاح.'];
        }
    ...
    public function status(Request $request, $id)
    {
        ...
        if ($user->status == Status::USER_ACTIVE) {
            ...
            $notify[] = ['success', 'تم حظر المستخدم بنجاح.'];
        } else {
            $user->status = Status::USER_ACTIVE;
            $user->ban_reason = null;
            $notify[] = ['success', 'تم الغاء حظر المستخدم بنجاح.'];
        }
        $user->save();
        return back()->withNotify($notify);
    }
    ...
    public function showNotificationSingleForm($id)
    {
        ...
        if (!gs('en') && !gs('sn') && !gs('pn')) {
            $notify[] = ['warning', 'خيارات الإشعارات معطّلة حالياً.'];
    ...
    public function sendNotificationSingle(Request $request, $id)
    {
        ...
        if (!$template) {
            $notify[] = ['warning', 'نموذج الإشعارات الافتراضي غير مُفعَّل'];
            return back()->withNotify($notify);
        }
    ...
    public function sendNotificationAll(Request $request)
    {
        ...
        $template = NotificationTemplate::where('act', 'DEFAULT')->where($request->via . '_status', Status::ENABLE)->exists();
        if (!$template) {
            $notify[] = ['warning', 'قالب الإشعارات الافتراضي غير مُفعَّل.'];
            return back()->withNotify($notify);
        }
        ...
        if ($request->being_sent_to == 'selectedUsers') {
            if (session()->has("SEND_NOTIFICATION")) {
                ...
                if (!$request->user || !is_array($request->user) || empty($request->user)) {
                    $notify[] = ['error', "تأكد من تعبئة حقل المستخدم عند إرسال بريد إلكتروني إلى مجموعة المستخدمين المعيّنة."];
                    return back()->withNotify($notify);
                }
            }
        }
        ...
        if ($totalUserCount <= 0) {
            $notify[] = ['error', "لم يتم العثور على مستلمي الإشعارات ضمن قاعدة المستخدمين المحددة."];
            return back()->withNotify($notify);
        }
