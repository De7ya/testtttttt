...
class ManageTripController extends Controller
{
    ...
    public function routeStore(Request $request)
    {
        ...
        if ($request->start_from == $request->end_to) {
            $notify[] = ['error', 'لا يمكن أن تكون نقطة البداية ونقطة النهاية متطابقتين.'];
        ...
        $notify[] = ['success', 'تم إنشاء المسار بنجاح'];
        return back()->withNotify($notify);
    }
    ...
    public function scheduleStore(Request $request, $id = 0)
    {
        ...
        if ($check) {
            $notify[] = ['error', 'تمّت إضافة هذا الجدول بالفعل.'];
    ...
    public function assignVehicle(Request $request, $id = 0)
    {
        ...
        if ($trip_check) {
            $notify[] = ['error', 'تم بالفعل تعيين مركبة لهذه الرحلة.'];
            return back()->withNotify($notify);
        }
        ...
        if ($vehicle_check) {
            $notify[] = ['error', 'تم بالفعل تعيين هذه المركبة إلى رحلة أخرى في هذا الوقت.'];
            return back()->withNotify($notify);
        }
        ...
        $notify[] = ['success', 'تم تعيين المركبة بنجاح.'];
        return back()->withNotify($notify);
    }
