...
class Deposit extends Model
{
    ...
    public function statusBadge(): Attribute
    {
        return new Attribute(function(){
            ...
            if($this->status == Status::PAYMENT_PENDING){
                $html = '<span class="badge badge--warning">'.trans('معلقة').'</span>';
            }
            ...
            elseif($this->status == Status::PAYMENT_SUCCESS && ($this->method_code < 1000 || $this->method_code >= 5000)){
                $html = '<span class="badge badge--success">'.trans('ناجحة').'</span>';
            }
            elseif($this->status == Status::PAYMENT_REJECT){
                ...
                $html = '<span class="badge badge--dark">'.trans('تم البدء').'</span>';
            }
            return $html;
        });
    }
    ...
    {
        return @$this->gateway->crypto == Status::ENABLE ? '' : $this->method_currency;
    }
