    ...
    public function statusBadge(): Attribute
    {
        return new Attribute(function(){
            ...
            if($this->status == Status::TICKET_OPEN){
                $html = '<span class="badge badge--success">'.trans("افتح").'</span>';
            }
            elseif($this->status == Status::TICKET_ANSWER){
                $html = '<span class="badge badge--primary">'.trans("الإجابة").'</span>';
            }

            elseif($this->status == Status::TICKET_REPLY){
                $html = '<span class="badge badge--warning">'.trans("ردّ العميل
").'</span>';
            }
            elseif($this->status == Status::TICKET_CLOSE){
                $html = '<span class="badge badge--dark">'.trans("مغلقة").'</span>';
            }
            return $html;
        });
    }
